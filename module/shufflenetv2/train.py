...
from module.shufflenetv2.shufflenetv2 import ShuffleNetV2Classifier
...
if __name__ == "__main__":
    ...
    # model
    model = ShuffleNetV2Classifier().to(device)
    ...
    for epoch in range(cfg["end_epoch"]):
        ...
        if top1 > best_top1:
            best_top1 = top1
            torch.save(model.state_dict(), str(savedir/f"{proj_name}_acc{top1}_ep{epoch}.pth"))
        if opt.enable_wandb:
